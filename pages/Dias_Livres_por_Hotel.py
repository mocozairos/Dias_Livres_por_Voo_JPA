...
st.title('Dias Livres por Hotel - João Pessoa')
...
df_in_out = pd.merge(df_in[['Reserva Mae', 'Servico', 'Voo', 'Horario Voo', 'Est Destino', 'Cliente', 'Telefone Cliente', 'Data Execucao']], df_out[['Reserva Mae', 'Data Execucao']], on='Reserva Mae', how='left')
...
if selected_rows is not None and len(selected_rows)>0:
    ...
    if servico!='Todos':
        ...
        df_ref_2 = df_ref = df_in_out[(df_in_out['Voo IN'].isin(selected_rows['Voo IN'].unique().tolist())) & 
                                      (df_in_out['Servico']==servico)].groupby('Est Destino')['Dias Livres'].sum().reset_index()
        ...
        if selected_rows_3 is not None and len(selected_rows_3)>0:
            ...
            df_ref_3 = df_in_out[(df_in_out['Voo IN'].isin(selected_rows['Voo IN'].unique().tolist())) & 
                                 (df_in_out['Servico']==servico) & 
                                 (df_in_out['Est Destino'].isin(selected_rows_3['Est Destino'].unique().tolist()))].reset_index(drop=True)
        
            df_ref_3['Data IN'] = pd.to_datetime(df_ref_3['Data IN']).dt.date

            df_ref_3['Data OUT'] = pd.to_datetime(df_ref_3['Data OUT']).dt.date
        ...
        if selected_rows_2 is not None and len(selected_rows_2)>0:
            ...
            df_ref_2 = df_ref = df_in_out[(df_in_out['Voo IN'].isin(selected_rows['Voo IN'].unique().tolist())) & 
                                          (df_in_out['Servico'].isin(selected_rows_2['Servico'].unique().tolist()))].groupby('Est Destino')['Dias Livres'].sum().reset_index()
            ...
            if selected_rows_3 is not None and len(selected_rows_3)>0:
                ...
                df_ref_3 = df_in_out[(df_in_out['Voo IN'].isin(selected_rows['Voo IN'].unique().tolist())) & 
                                     (df_in_out['Servico'].isin(selected_rows_2['Servico'].unique().tolist())) & 
                                     (df_in_out['Est Destino'].isin(selected_rows_3['Est Destino'].unique().tolist()))].reset_index(drop=True)
